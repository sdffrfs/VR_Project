...
分为ForceController、OppositeDirController和UnidirController三个模块，三者之间相互独立，分别测试了不同的功能。
...
1. 安装 CLion（个人习惯 CLion 作为 IDE，也可以使用 VSCode，或者 Arduino 原生 IDE）：https://www.jetbrains.com/clion/
2. 安装 PlatformIO：教程地址：【从零学习 Arduino Uno R4 配套教程 CLion（持续更新中）】https://www.bilibili.com/video/BV1ku411j7oS?vd_source=72f0056fa0df6b22791b0b410c0bcae1
...
2. 安装 Rider（个人习惯 Rider 作为编程 IDE，也可以使用 VS，VSCode）：https://www.jetbrains.com/rider/
...
### SerialController（抽象基类）：提供串口通信的基础功能，所有业务逻辑继承此基类。
主要功能:
- 串口自动连接和重连
...
1. **串口连接**: 确保 ESP32 连接到正确的 COM 端口
2. **电压安全**: 电机供电电压必须与配置一致（12V）
3. **电机校准**: 首次运行时需要进行电机校准（FOC 初始化）
4. **碰撞检测**: ForceController 需要在场景中添加名为 "Wall" 的碰撞体
5. **线程安全**: Unity 串口通信使用多线程，注意数据同步
